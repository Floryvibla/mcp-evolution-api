...
class EvolutionMCPServer {
  ...
  private async handleSendTemplate(args: any) {
    ...
    // Validate variables
    const missingVars = templateService.validateVariables(
      template,
      args.variables,
    );
      ...
      content: [
        {
          ...
          text: deleted
            ...
            : "Cannot delete default template or template not found",
        },
      ],
    };
  }
  ...
  private async handleCheckNumber(args: any) {
    const result = await evolutionAPI.checkNumberStatus(
      args.instanceName,
      args.numbers,
    );
    return {
  ...
  private async handleRemoveParticipants(args: any) {
    const result = await evolutionAPI.removeGroupParticipants(
      args.instanceName,
      {
        groupJid: args.groupJid,
        participants: args.participants,
...
const handleSSEConnection = async (
  ...
  try {
    ...
    transport.onerror = (error: Error) => {
      logger.error("❌ MCP SSE session error", {
        error,
        sessionId: transport.sessionId,
      });
    };
  } catch (error) {
    logger.error("❌ Failed to start MCP SSE session", { error });
    if (!res.headersSent) {
...
async function main() {
  ...
  app.listen(PORT, () => {
    logger.info(`🌐 Webhook server started`, {
      ...
      mcpSseEndpoint: `http://localhost:${PORT}/mcp/sse`,
    });
    ...
    console.log("  GET  /mcp/sse - Connect to MCP via SSE");
    console.log("  POST /mcp/message - Send MCP messages");
    console.log("\n🚀 Server ready!");
  });
}
